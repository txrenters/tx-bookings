...
|------|-----|--------------|
| Primary | `#304364` | `--color-primary` |
| On Primary | `#FFFFFF` | `--color-on-primary` |
| Secondary | `#6B87BC` | `--color-secondary` |
| On Secondary | `#0F172A` | `--color-on-secondary` |
| Accent/CTA | `#527A22` | `--color-accent` |
| On Accent/CTA | `#FFFFFF` | `--color-on-accent` |
| Background | `#F8FAFC` | `--color-background` |
| Foreground | `#1A2436` | `--color-foreground` |
| Card | `#FFFFFF` | `--color-card` |
| Card Foreground | `#1A2436` | `--color-card-foreground` |
| Muted | `#F1F4F9` | `--color-muted` |
| Muted Foreground | `#4E5A70` | `--color-muted-foreground` |
| Border | `#E2E7F0` | `--color-border` |
| Destructive | `#DC2626` | `--color-destructive` |
| On Destructive | `#FFFFFF` | `--color-on-destructive` |
| Ring | `#304364` | `--color-ring` |

**Color Notes:** Derived from the TexaRenters brand mark in `public/logo.png`, not from the generator's stock SaaS palette. Navy `#304364` (9.9:1 on white) is the single action colour: buttons, active nav, the selected calendar day. The logo's lime `#92C14F` is too light to carry text or act as an indicator (2.1:1 on white), so availability and confirmation states use the darkened `#527A22` (5.0:1 on white) and the `success-*` token set. Never recolour the logo itself — it stays full-colour on a neutral surface.
