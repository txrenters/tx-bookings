...
test('a slug held by a soft-deleted event type is suffixed on create', function () {
    $trashed = EventType::factory()->ownedBy($this->user)->create(['slug' => 'discovery-call']);
    $trashed->delete();
    ...
    expect(EventType::sole()->slug)->toBe('discovery-call-2');
    expect($trashed->fresh()->slug)->toBe('discovery-call');
});

test('a slug held by a soft-deleted event type is suffixed on update', function () {
    $eventType = EventType::factory()->ownedBy($this->user)->create(['slug' => 'discovery-call']);
    $trashed = EventType::factory()->ownedBy($this->user)->create(['slug' => 'archived-call']);
    $trashed->delete();
    ...
    $this->actingAs($this->user)
        ...
        ]), eventTypePayload(['slug' => 'archived-call']))
        ->assertSessionHasNoErrors();

    expect($eventType->fresh()->slug)->toBe('archived-call-2');
});
...
test('a duplicated slug steps past soft-deleted event types', function () {
    $eventType = EventType::factory()->ownedBy($this->user)->create(['slug' => 'discovery-call']);
    $trashed = EventType::factory()->ownedBy($this->user)->create(['slug' => 'discovery-call-2']);
    $trashed->delete();
